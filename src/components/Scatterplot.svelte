<script>
  ...
  // Optional: force axis domain. Falls back to data extent.
  ...
  $: xScale = d3.scaleLinear()
    .domain(xDomain || d3.extent(data, d => d[xVar]))
    .nice()
    .range([0, chartW]);

  $: yScale = d3.scaleLinear()
    .domain(yDomain || d3.extent(data, d => d[yVar]))
    .nice()
    .range([chartH, 0]);
...
<svg width="100%" height="100%">
  <g transform="translate({margin.left}, {margin.top})">
    ...
    <!-- Background annotations (behind points) -->
    <g class="annotations">
      {#each annotations.filter(a => a.behind) as ann}
        <g in:fade={{ duration: 500 }}>
          <line
            x1={xScale(ann.x1)} y1={yScale(ann.y1)}
            x2={xScale(ann.x2)} y2={yScale(ann.y2)}
            stroke={ann.stroke || "#333"}
            stroke-width={ann.strokeWidth || 1.5}
            stroke-dasharray={ann.strokeDasharray || "5 3"}
          />
        </g>
      {/each}
    </g>
    ...
    <g class="annotations">
      {#each annotations.filter(a => !a.behind) as ann}
        <g in:fade={{ duration: 500, delay: 1000 }}>
          <line
            x1={xScale(ann.x1)} y1={yScale(ann.y1)}
            x2={xScale(ann.x2)} y2={yScale(ann.y2)}
            stroke={ann.stroke || "#333"}
